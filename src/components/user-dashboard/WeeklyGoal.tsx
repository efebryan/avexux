...
const DepositModal = dynamic(
  () =>
    import("@/components/user-dashboard/wallet/DepositModal").then(
      (mod) => mod.DepositModal,
    ),
  {
    ssr: false,
  },
...
export function WeeklyGoal() {
      ...
      if (user) {
        ...
        if (activeTasks) {
          ...
          const doneBeforeTodayIds = new Set(
            (allSubmissions || [])
              .filter((sub) => {
                ...
                const isDone =
                  sub.status === "Approved" || sub.status === "Pending Review";
          ...
          const targetSum = activeTasks
            ...
            .reduce(
              (sum: number, task: any) => sum + Number(task.reward_amount || 0),
              0,
            );
        ...
        if (allSubmissions) {
          ...
          const achievedSum = allSubmissions
            ...
            .reduce((sum: number, sub: any) => {
              const taskRef = activeTasks?.find(
                (t: any) => t.id === sub.task_id,
              );
  ...
  const handleDeposit = (amount: number, method: string) => {
    toast.success(
      `Successfully upgraded plan with ₦${amount.toLocaleString()}!`,
    );
    setIsUpgradeModalOpen(false);
  };
  const progressPercentage =
    dailyTarget > 0
      ? Math.min(100, Math.round((achievedToday / dailyTarget) * 100))
      : 0;
    ...
    <Card className="p-3.5 border border-gray-100 shadow-sm rounded-xl mb-4">
      ...
      <div className="relative flex justify-center mb-4">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-xl font-bold text-[#0f8538]">
            {progressPercentage}%
          </span>
      ...
      <div className="space-y-2 mb-4">
        <div className="flex justify-between items-center text-xs">
          ...
          <span className="font-bold text-gray-900">
            ₦
            {dailyTarget.toLocaleString(undefined, {
        ...
        <div className="flex justify-between items-center text-xs">
          ...
          <span className="font-bold text-[#0f8538]">
            ₦
            {achievedToday.toLocaleString(undefined, {
              minimumFractionDigits: 2,
              maximumFractionDigits: 2,
            })}
          </span>
