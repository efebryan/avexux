    ...
    <Card className="p-5 max-w-xl mx-auto rounded-xl border border-gray-100 shadow-lg bg-white flex flex-col items-center relative overflow-hidden">
      <div className="text-center mb-6">
        <h2 className="text-xl font-extrabold text-gray-900 flex items-center justify-center gap-1.5">
          <Trophy className="w-5 h-5 text-yellow-500" /> Lucky Rewards Spin
          Wheel
        </h2>
        <p className="text-xs text-gray-500 mt-1 max-w-sm">
          Spin the wheel of fortune! Win instant cash bonuses, premium status,
          and gift card packages.
        </p>
      </div>
      ...
      <div className="w-full flex flex-col items-center gap-3">
        <div className="flex gap-4 text-xs font-bold text-gray-600 bg-slate-50 px-4 py-2 rounded-lg border border-slate-100 shadow-inner">
          <span className="flex items-center gap-1">
            <RefreshCw className="w-3.5 h-3.5 text-blue-500" /> Free Spins:{" "}
            {spinsLeft}
          </span>
          <span className="flex items-center gap-1">
            <Coins className="w-3.5 h-3.5 text-yellow-500" /> Spin Cost: ₦
            {spinCost}
          </span>
        </div>
        ...
        <Button
          onClick={spin}
          disabled={
            isSpinning || !canSpinToday || (spinsLeft <= 0 && hasUsedPaidSpin)
          }
              ...
              {resultSector.isWin ? (
                <>
                  ...
                  <DialogDescription className="text-xs text-gray-600 mt-2">
                    ...
                    !
                    {resultSector.value
                      ? ` The bonus of ₦${resultSector.value.toLocaleString()} has been credited to your wallet.`
                      : " Your prize package is being set up."}
                  </DialogDescription>
                </>
              ) : (
                <>
                  ...
                  <DialogDescription className="text-xs text-gray-600 mt-2">
                    Oh! You landed on {resultSector.label}. Don't worry, try
                    again on your next available spin!
                  </DialogDescription>
                </>
              )}
            </div>
