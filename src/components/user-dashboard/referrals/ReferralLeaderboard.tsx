    ...
    <div className="bg-white border border-gray-100 rounded-3xl shadow-sm overflow-hidden animate-in fade-in duration-500">
      <div className="p-6 border-b border-gray-100 bg-[#f8fafc]">
        <h3 className="text-xl font-bold text-gray-900 flex items-center gap-2">
          <Trophy className="w-6 h-6 text-[#0f8538]" /> Monthly Leaderboard
        </h3>
        <p className="text-sm text-gray-500 mt-1">Top referrers this month. Can you make it to the top?</p>
      </div>
            ...
            {leaders.map((leader) => (
              <tr key={leader.rank} className={`hover:bg-gray-50/50 transition-colors ${leader.rank <= 3 ? "bg-yellow-50/20" : ""}`}>
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center ${leader.rank <= 3 ? "bg-white shadow-sm border border-yellow-100" : ""}`}>
                    {getRankIcon(leader.rank)}
                  </div>
                </td>
                <td className="px-6 py-4 font-bold text-gray-900">{leader.username}</td>
                <td className="px-6 py-4 text-center font-medium text-gray-600">{leader.totalInvites}</td>
