        ...
        <table className="w-full text-xs text-left">
          ...
          <tbody className="divide-y divide-gray-100">
            {withdrawals.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-4 py-6 text-center text-gray-500">No withdrawals yet.</td>
              </tr>
            ) : (
              withdrawals.map((w) => (
                <tr key={w.id} className="hover:bg-gray-50/50 transition-colors">
                  <td className="px-4 py-2.5 whitespace-nowrap text-gray-500">{w.date}</td>
                  <td className="px-4 py-2.5 font-medium text-gray-900">{w.method}</td>
                  <td className="px-4 py-2.5 text-gray-500">{w.accountDetails}</td>
                  <td className="px-4 py-2.5 whitespace-nowrap font-bold text-gray-900">
                  ...
                  <td className="px-4 py-2.5 whitespace-nowrap">
                    ...
                    {w.status === "Failed" && (
                      <span className="flex items-center gap-1 text-red-600 bg-red-50 px-1.5 py-0.5 rounded text-[9px] font-bold w-fit">
