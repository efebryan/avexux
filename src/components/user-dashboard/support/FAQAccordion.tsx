    ...
    <div>
      ...
      <div className="space-y-2.5">
        {faqs.map((faq, i) => (
          <div 
            key={i} 
            className={`border rounded-xl overflow-hidden transition-colors ${openIndex === i ? 'border-[#0f8538]/30 bg-[#f8fafc]' : 'border-gray-200 bg-white hover:border-gray-300'}`}
          >
            <button
              onClick={() => setOpenIndex(openIndex === i ? null : i)}
              className="w-full flex items-center justify-between p-3.5 text-left focus:outline-none"
            >
              <span className={`font-bold text-sm pr-4 ${openIndex === i ? 'text-[#0f8538]' : 'text-gray-900'}`}>
                {faq.question}
              </span>
              <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 transition-transform ${openIndex === i ? 'bg-[#ade5bb]/40 rotate-180' : 'bg-gray-100'}`}>
                <ChevronDown className={`w-3 h-3 ${openIndex === i ? 'text-[#0f8538]' : 'text-gray-500'}`} />
              </div>
            </button>
            <div 
              className={`px-3.5 overflow-hidden transition-all duration-300 ease-in-out ${
                openIndex === i ? "max-h-48 pb-3.5 opacity-100" : "max-h-0 opacity-0"
              }`}
            >
