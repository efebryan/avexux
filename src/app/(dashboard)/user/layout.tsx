...
export default function UserDashboardLayout({
  ...
  return (
    <div className="flex h-[100dvh] w-full bg-[#f8fafc] overflow-hidden relative">
      ...
      {/* Mobile Sidebar */}
      <div className={`fixed inset-y-0 left-0 z-50 w-[280px] bg-[#f8fafc] border-r border-gray-200 shadow-2xl transform transition-transform duration-300 lg:hidden ${isMobileSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="absolute top-6 right-4 z-50">
          <button onClick={() => setIsMobileSidebarOpen(false)} className="p-2 hover:bg-gray-200 rounded-full text-gray-500 transition-colors">
            <X className="w-6 h-6" />
          </button>
        </div>
      ...
      <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
        {/* Header */}
        <header className="h-20 shrink-0 border-b border-gray-200 bg-[#f8fafc] px-4 md:px-6 flex items-center gap-4">
          <button 
