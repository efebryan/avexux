    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-2">
        <div>
          ...
          <p className="text-gray-500 text-sm">Create tasks and review user submissions.</p>
        </div>
        {activeTab === "manage" && (
      ...
      <div className="flex items-center gap-6 border-b border-gray-200 mb-6">
        <button 
          ...
          className={`pb-4 text-sm font-bold border-b-2 transition-colors ${activeTab === "manage" ? "border-blue-600 text-blue-600" : "border-transparent text-gray-500 hover:text-gray-700"}`}
        >
          Manage Tasks
        </button>
        <button 
          onClick={() => setActiveTab("review")}
          className={`pb-4 text-sm font-bold border-b-2 transition-colors flex items-center gap-2 ${activeTab === "review" ? "border-blue-600 text-blue-600" : "border-transparent text-gray-500 hover:text-gray-700"}`}
        >
          Review Submissions
                ...
                {tasks.map((task) => (
                  <tr key={task.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4">
                      ...
                      <p className="text-xs text-gray-500">{task.advertiser} • {task.created}</p>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 font-medium text-gray-700">
                      {task.submissions} completed
                    </td>
                    <td className="px-6 py-4">
                      <button 
                        onClick={() => toggleTaskStatus(task.id)}
                        className={`text-xs font-bold px-2 py-1 rounded transition-colors ${
                          task.status === "Active" ? "bg-green-100 text-green-700 hover:bg-green-200" : "bg-yellow-100 text-yellow-700 hover:bg-yellow-200"
                        }`}
                      >
                        {task.status}
                      </button>
            ...
            <table className="w-full text-sm text-left">
              <thead className="text-xs text-gray-500 uppercase bg-gray-50 border-b border-gray-100">
                <tr>
                  ...
                  <th className="px-6 py-4 font-semibold">Status</th>
                  <th className="px-6 py-4 font-semibold text-right">Review Proof</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50">
                {submissions.map((sub) => (
                  <tr key={sub.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-gray-700">{sub.taskTitle}</td>
                    <td className="px-6 py-4 text-gray-500 text-xs">{sub.date}</td>
                    <td className="px-6 py-4">
                      ...
                      {sub.status === "Pending" ? (
                        <div className="flex items-center justify-end gap-2">
                          <Button 
                            variant="outline" 
                            size="sm" 
                            className="h-8 text-xs font-bold text-blue-600 border-blue-200 hover:bg-blue-50 flex gap-1"
                          >
