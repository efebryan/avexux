...
export default function AdminRewardsPage() {
  ...
  const [spinsPerPlanConfig, setSpinsPerPlanConfig] = useState<
  ...
  const [sectorForm, setSectorForm] = useState<any>({
    id: "",
    label: "",
    type: "cash",
    value: 0,
  ...
  const handleSaveModalConfig = async () => {
    ...
    if (error) {
      ...
      toast.error(
        `Failed to save popup configuration: ${error.message || JSON.stringify(error)}`,
      );
    } else {
  ...
  const handleRigSpin = async () => {
    ...
    if (error) {
      ...
      toast.success(
        `Spin successfully rigged for ${selectedUser.full_name || selectedUser.email}!`,
      );
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-2">
        <div>
          ...
          <p className="text-gray-500 text-sm">
            Manage the Spin Wheel and Dashboard Popup.
          </p>
        </div>
      </div>
        ...
        <Card className="p-6 md:p-8 border border-gray-100 shadow-sm rounded-2xl bg-white relative overflow-hidden">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Configure what users can win.
                </p>
              </div>
            </div>
            <Button
              onClick={handleSaveWheelConfig}
              className="bg-blue-600 hover:bg-blue-700 text-white font-medium flex items-center gap-2 rounded-xl shadow-md shadow-blue-500/20 hover:-translate-y-0.5 transition-all"
            >
          ...
          <div className="mt-5 relative z-10">
            <Button
              ...
              variant="outline"
              className="w-full border-dashed border-gray-300 text-gray-500 hover:text-blue-600 hover:bg-blue-50 hover:border-blue-300 rounded-xl h-12 transition-all"
            >
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4 relative z-10">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Guarantee a specific prize for a user's next spin.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10 bg-purple-50/50 p-6 rounded-xl border border-purple-100">
            ...
            <div>
              ...
              <div className="mt-4 flex justify-end">
                <Button
                  onClick={handleRigSpin}
                  disabled={!selectedUser || !selectedRigSector}
                  className="bg-purple-600 hover:bg-purple-700 text-white font-medium flex items-center gap-2 rounded-xl shadow-md transition-all w-full md:w-auto"
                >
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4 relative z-10">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                Manage the modal shown on user dashboard load.
              </p>
            </div>
            <div className="flex items-center gap-4">
              <button
              ...
              <Button
                ...
                className="bg-slate-900 hover:bg-slate-800 text-white font-medium flex items-center gap-2 h-10 rounded-xl shadow-md"
              >
        ...
        <Card className="p-6 md:p-8 border border-gray-100 shadow-sm rounded-2xl bg-white relative overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4 relative z-10">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                Configure which days of the week each membership plan can spin.
              </p>
            </div>
            <Button
              onClick={handleSavePlanConfig}
              className="bg-emerald-600 hover:bg-emerald-700 text-white font-medium flex items-center gap-2 h-10 rounded-xl shadow-md"
            >
            ...
            {Object.keys(planLabels).map((plan) => (
              ...
              >
                ...
                <div className="flex flex-wrap gap-2">
                  {daysOfWeek.map((day) => {
                    const isChecked =
                      spinsPerPlanConfig[plan]?.includes(day.id) || false;
                    return (
                      <label
                        key={day.id}
                        className="flex items-center gap-2 bg-white px-3 py-1.5 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors"
                      >
                        <Checkbox
                          checked={isChecked}
          ...
          <form onSubmit={handleSaveSector} className="p-6 space-y-4">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setSectorForm({ ...sectorForm, label: e.target.value })
                }
                className="w-full px-4 py-2.5 border rounded-xl text-sm focus:ring-2 focus:ring-blue-500/20 bg-gray-50"
              />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select
                  value={sectorForm.type}
                  onChange={(e) =>
                    setSectorForm({ ...sectorForm, type: e.target.value })
                  }
                  className="w-full px-4 py-2.5 border rounded-xl text-sm bg-gray-50"
                >
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    onChange={(e) =>
                      setSectorForm({ ...sectorForm, color: e.target.value })
                    }
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border rounded-xl text-sm bg-gray-50 uppercase"
                  />
                </div>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  value={sectorForm.isWin ? "true" : "false"}
                  onChange={(e) =>
                    setSectorForm({
                      ...sectorForm,
